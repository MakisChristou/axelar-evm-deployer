//! Pure encoding/derivation helpers for Solana ITS: PDA seeds, ATA address
//! derivation, the interchain-token-id digest, and keypair loading. Nothing
//! here touches an RPC — the `gateway` and `its` modules sit on top of these
//! and add the network calls.
...
pub(super) fn find_token_manager_pda(its_root: &Pubkey, token_id: &[u8; 32]) -> (Pubkey, u8) {
